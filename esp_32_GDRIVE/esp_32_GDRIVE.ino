...
//
//String myScript = "/macros/s/YOUR SCRIPT ID/exec";//Create your Google Apps Script and replace the "myScript" path.
// INSTEAD OF DECALRING THE PERSONAL INFO IS A GOOD PRACTICE TO DECALRE IT ANOTHER FILE NAMED CREDENTIAL.H
...
void loop()
{
  ...
  delay(10000);
  Serial.println("50 seconds left");
  delay(10000);
  Serial.println("40 seconds left");
  delay(10000);
  Serial.println("30 seconds left");
  delay(10000);
...
String SendCapturedImage() {
  ...
  for(int i = 0; i<5 ;i++)
  {
    ...
    {
      is_connected = true;
      Serial.println("attempt"+ String(i) +"Sucess");
      break;
    }else{
      is_connected = false;
      Serial.println("attempt"+ String(i) +"failed");
      delay(1000);
    }
  }
  if (is_connected) {
